...
public abstract class ParticleRing {
	public ParticleRing(final Location centerLoc, final ParticleEffects type,
			final long delay) {
		...
		for (int i = 0; i < circleElements; ++i) {
			...
			final Location particle = new Location(centerLoc.getWorld(),
					centerLoc.getX() + x, centerLoc.getY(), centerLoc.getZ()
							+ z);
