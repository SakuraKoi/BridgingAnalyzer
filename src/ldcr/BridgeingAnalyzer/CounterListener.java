...
public class CounterListener implements Listener {
	...
	public void onClick(final PlayerInteractEvent e) {
		if (e.getAction().toString().contains("CLICK")) {
			...
			ActionBarUtils.sendActionBar(
					e.getPlayer(),
					"§c§l最大CPS - " + c.getMaxCPS() + " §d§l当前CPS - "
							+ c.getCPS() + " §a§l| §c§l最远距离 - "
							+ c.getMaxBridgeLength() + " §d§l当前距离 - "
							+ c.getBridgeLength());
		}
	}
	...
	public void onFallDown(final PlayerMoveEvent e) {
		if (e.getTo().getY() < 0) {
			...
			if (c.isSpeedCountEnabled()) {
				Util.f__kUnknownTitleFadeBug(e.getPlayer(), "",
						"§cMax - " + c.getMaxBridgeSpeed() + " block/s", 0,
						100, 0);
			}
			c.reset();
	...
	public void onPlaceBlock(final BlockPlaceEvent e) {
		...
		if (e.getPlayer() != null) {
			...
			if (c.isSpeedCountEnabled()) {
				Util.f__kUnknownTitleFadeBug(e.getPlayer(), "",
						"§b" + c.getBridgeSpeed() + " block/s", 0, 100, 0);
			}
			Bukkit.getScheduler().runTaskLater(BridgingAnalyzer.instance,
				...
				public void run() {
					...
					.getInventory()
					.addItem(
							new ItemStack(e.getPlayer()
									.getItemInHand().getType(),
									1, (short) 0, e.getPlayer()
									.getItemInHand()
									.getData()
									.getData()));
				}
