...
public class BridgeCommand implements CommandExecutor {
	...
	if (args.length != 2) {
	    SendMessageUtils.sendMessage(sender, "§6§l[BridgingAnalyzer] §b骚操作练习插件 §lBy.Ldcr",
	...
	case "highlight": {
	    ...
	    sender.sendMessage("§6§l[BridgingAnalyzer] §a侧搭辅助指示已 " + (counter.isHighlightEnabled() ? "开启" : "关闭"));
	    break;
	}
	case "damage": {
	    counter.setDamageEnabled(toBoolean(args[1]));
	    sender.sendMessage("§6§l[BridgingAnalyzer] §a伤害已 " + (counter.isDamageEnabled() ? "开启" : "关闭"));
	    break;
	}
	case "speed": {
	    counter.setSpeedCountEnabled(toBoolean(args[1]));
	    sender.sendMessage("§6§l[BridgingAnalyzer] §a搭路速度统计已 " + (counter.isSpeedCountEnabled() ? "开启" : "关闭"));
	    break;
	}
	}
	return true;
